...
export default function Lista03() {
    const frutas = ["Banana", "Maça", "Abacaxi", "Uva verde"];
    const produtos = [
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.titulo}>Lista 03 - Enzo Antônio Ferreira de Araújo =D</Text>
      <View style={styles.card}>
        <Text style={styles.label}>Exercício 1 - Lista de Frutas</Text>
            <Text>{frutas.map((frutas, index) => (
                <Text key={index}>
                    {index + 1}-{frutas}.
                </Text>
            ))}
      ...
      <View style={styles.card}>
        ...
        <Text>{produtos.map((produtos) => (
          <Text key={produtos.id}>
            {produtos.nome} | Preço: {produtos.preco}...
          </Text>
        ))}
        </Text>
      </View>
